const colors = {
  primary: {
    primaryColor: 'rgba(242, 242, 242, 1)',
    primaryColorWhite: 'rgba(255, 255, 255, .55)',
    primaryColorTextShadow: '2px 2px 2px rgba(42, 51, 112, 1)',
    headerBodyTextColor: 'rgb(42, 51, 112)',
    linkColor: 'rgba(42, 51,112,1)',
    activeLinkColor: 'rgba(255, 240, 25,1)',
    hoverColor: 'rgba(171, 237, 253, 1)',
    iconColor: 'rgba(255, 240, 25,1)',
    activeIconColor: 'rgba(252, 252, 252, 1)',
    backgroundColor: 'rgba(252, 252, 252, .85)',
    searchBoxBackgroundColor: 'rgba(42, 51, 112, .85)',
    boxShadowColor: '-2px -2px rgba(42, 51, 112, .45)',
  },
  secondary: {
    secondaryColor: 'rgba(28, 34, 38,1)',
    secondaryColorTextShadow: '1px 1px 1px rgba(42, 51, 112, .35)',
    secondaryActiveIconColor: 'rgba(42, 51, 112, 1)',
  },
};

export default colors;

/*$osebro-antikvariat-black: rgba(28, 34, 38, 1);
$osebro-antikvariat-white: rgba(242, 242, 242, 1);
$osebro-antikvariat-green: rgba(255, 240, 25,1);
$osebro-antikvariat-lightblue: rgba(171, 237, 253, 1);
$osebro-antikvariat-blue: rgba(42, 51, 112, 1);*/
